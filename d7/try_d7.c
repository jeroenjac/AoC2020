...
void	storedata(char *file, int lines, struct ds *groups, struct cs ***colchains);
int		getnumberoflines(char *file, int *ptparts);
void	ft_lstadd_back(tcs **lst, tcs *new);
tcs	*ft_lstnew(void *content);
...
int		main(void)
{
	...
	colchains = (tcs ***)calloc(lines, sizeof(tcs));
	if (colchains == NULL)
		printf("Allocation for data array failed");

	//Step 3: Reading file per MAXBUF chars
	printf("Start reading input...\n");
	storedata(file, lines, colorlines, colchains);
...
void		storedata(char *file, int lines, struct ds *dat, struct cs ***colchains)
{
	FILE	*in_file;
	char	line[MAXBUF];
	char	*pt, *end;
	...
	while (fgets(line, MAXBUF, in_file) != NULL)
	{
		...
		while (*pt != '.' && pt != NULL)
		{
			...
			newcolor = strndup(pt + 3, end - pt);
			ft_lstadd_back(colchains[i], ft_lstnew(newcolor));
			pt = strchr(pt, ',');
		}
		i++;
	}
